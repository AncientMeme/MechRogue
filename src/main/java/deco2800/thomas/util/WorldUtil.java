...
public class WorldUtil {
	...
	public static float[] worldCoordinatesToColRow(float x, float y) {
		...
		float col;
		float actualRow;
		float scaledWidth = TILE_WIDTH * 0.75f;
		x -= scaledWidth / 2;
		y -= TILE_HEIGHT / 2;
		col = Math.round(x / scaledWidth);
	...
	public static boolean isWalkable(float col, float row) {
  	...
  	public static boolean areCoordinatesOffScreen(float squareX, float squareY, OrthographicCamera camera) {
		float bufferShift = 110.0f;

		if (squareX > camera.position.x) {
			squareX -= bufferShift;
		} else {
			squareX += bufferShift;
		}

		if (squareY > camera.position.y) {
			squareY -= bufferShift;
		} else {
			squareY += bufferShift;
		}

		return !camera.frustum.pointInFrustum(squareX, squareY, 0.0f);
  	}
