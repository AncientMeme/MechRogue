...
public class SquareVectorTest {
    ...
    /**
     * Tests to make sure that randomDistance only returns Squarevectors
     * within provided range of original
     * */
    @Test
    public void testRandomDistanceWithinRange() {
        SquareVector vector = new SquareVector(1, 1);
        SquareVector nearbyVector = vector.randomDistance(10f);
        assertEquals(true,
                nearbyVector.getCol() <= vector.getCol()+10f &&
                        nearbyVector.getCol() >= vector.getCol()-10f
        );
        assertEquals(true,
                nearbyVector.getCol() <= vector.getRow()+10f &&
                        nearbyVector.getCol() >= vector.getRow()-10f
        );
    }
